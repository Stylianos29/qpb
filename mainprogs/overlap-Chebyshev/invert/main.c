...
main(int argc, char *argv[])
{
  ...
  int N_Cheb;
  if(sscanf(qpb_parse("Number of Chebyshev terms"), "%d", &N_Cheb)!=1)
    {
      error("error parsing for %s\n",
  ...
  qpb_double solver_epsilon;
  if(sscanf(qpb_parse("Solver epsilon"), "%lf", &solver_epsilon)!=1)
    {
      error("error parsing for %s\n",
	    "Solver epsilon");
      exit(QPB_PARSER_ERROR);
    }
  int solver_max_iters;
  if(sscanf(qpb_parse("Solver max iters"), "%d", &solver_max_iters)!=1)
    {
      error("error parsing for %s\n",
	    "Solver max iters");
      exit(QPB_PARSER_ERROR);
