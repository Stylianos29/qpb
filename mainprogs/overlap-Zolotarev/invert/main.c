...
main(int argc, char *argv[])
{
  ...
  int Zol_order;
  if(sscanf(qpb_parse("Zolotarev order"), "%d", &Zol_order)!=1)
    {
      error("error parsing for %s\n", 
	    "Zolotarev order");
      exit(QPB_PARSER_ERROR);
  ...
  if(Zol_order<1)
    {
      error("only provide positive integer values for Zolotarev order, quitting\n");
  ...
  if(sscanf(qpb_parse("Outer solver"), "%s", aux_string)!=1)
    {
      error("error parsing for %s\n", 
	    "Outer solver");
      exit(QPB_PARSER_ERROR);
  ...
  if(strcmp(aux_string, "bicgstab") == 0)
    ...
    {
      error("%s: option should be one of: ", "Outer solver");
      error("%s, ", "bicgstab"); 
      error("%s, ", "cg"); 
      error("%s, ", "bicgg5");
      error("%s\n", "mscg"); 
  ...
  qpb_double outer_epsilon;
  if(sscanf(qpb_parse("Outer solver epsilon"), "%lf", &outer_epsilon)!=1)
    {
      error("error parsing for %s\n",
	    "Outer solver epsilon");
      exit(QPB_PARSER_ERROR);
    }
  int outer_max_iters;
  if(sscanf(qpb_parse("Outer solver max iters"), "%d", &outer_max_iters)!=1)
    {
      error("error parsing for %s\n",
	    "Outer solver max iters");
      exit(QPB_PARSER_ERROR);
    }
  qpb_double ms_epsilon;
  if(sscanf(qpb_parse("Inner solver epsilon"), "%lf", &ms_epsilon)!=1)
    {
      error("error parsing for %s\n",
	    "Inner solver epsilon");
      exit(QPB_PARSER_ERROR);
    }
  int ms_max_iters;
  if(sscanf(qpb_parse("Inner solver max iters"), "%d", &ms_max_iters)!=1)
    {
      error("error parsing for %s\n",
  ...
  if(sscanf(qpb_parse("Max eigenvalue squared modification"), "%lf", &delta_max)!=1)
    {
      error("error parsing for %s\n", "Max eigenvalue squared modification");
      exit(QPB_PARSER_ERROR);
    }

  qpb_double delta_min;
  if(sscanf(qpb_parse("Min eigenvalue squared modification"), "%lf", &delta_min)!=1)
    {
      error("error parsing for %s\n", "Min eigenvalue squared modification");
      exit(QPB_PARSER_ERROR);
    }
