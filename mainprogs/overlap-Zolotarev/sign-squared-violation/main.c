...
#pragma omp parallel
  ...
  for(int i=0; i<n_vec; i++)
  {
    ...
    qpb_gamma5_sign_function_of_X_Zolotarev(signX_squared, eta[i]);
    qpb_spinor_gamma5(signX_squared, signX_squared);

    qpb_gamma5_sign_function_of_X_Zolotarev(temp, signX_squared);
    ...
    print(" Done vector = %d / %d, || Sign^2(X) - 1 ||^2 = %.25e\n", i+1, n_vec,\
                                                                  diffs[i]);
  }
  t = qpb_stop_watch(t);
