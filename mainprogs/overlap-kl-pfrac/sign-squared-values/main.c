...
#pragma omp parallel
  ...
  for(int i=0; i<n_vec; i++)
  {
    ...
    qpb_gamma5_sign_function_of_X_pfrac(signX, eta[i]);
    qpb_spinor_gamma5(signX, signX);

    qpb_double signX_norm, eta_norm;
    qpb_spinor_xdotx(&signX_norm, signX);
    qpb_spinor_xdotx(&eta_norm, eta[i]);

    diffs[i] = signX_norm/eta_norm;
  ...
  print(" || Sign^2(X) ||^2 values (per stochastic source):\n");
  for(int i=0; i<n_vec; i++)
    print(" %4d %e\n", i, diffs[i]);
  free(diffs);
